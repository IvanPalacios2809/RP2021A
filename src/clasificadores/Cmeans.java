...
public class Cmeans implements ClasificadorSupervisado{
    ...
    for(int i=0;i<PC.size();i++){
        ...
        while(PC.get(i).media!=PC.get(i).media_parcial){//Se busca romper el ciclo en dado caso de que la media no cambie     
            ...
            PC.get(i).sacarDistancia(instancias);//Sacamos las distancias y las alojamos en un vector de el patron cmeans en cuestion
            PC.get(i).ordenar();//Ordenamos las distancias
            PC.get(i).imprimir_orden(mitades);
            PC.get(i).posicionar(mitades,instancias);//Posicionamos el nuevo punto del Patron Cmeans en cuestion
            PC.get(i).media=0;
            PC.get(i).sacarMedia(mitades);//Sacamos la nueva media
  //          if(i>0 && PC.get(i).media==PC.get(i-1).media){
